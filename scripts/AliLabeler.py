...
# query 0 # target 1 # fident 2 # alnlen 3 # mismatch 4 # gapopen 5 # qstart 6 # qend 7 # tstart 8 # tend 9 # qlen 10 # tlen 11 # evalue 12 # bits 13 # alntmscore 14 #lddt 15

def PreparingAliDF(AliRawDf):
    AliRawDf.columns = "query target fident alnlen mismatch gapopen qstart qend tstart tend qlen tlen evalue bits alntmscore lddt".split()
...
def FindingLabels(FoldSeekRawOutput, PfamFile, FoldSeekLabeledOutput): #it takes the address of the pfam as input   
    ...
    FalsPosInds = (merged_df["PredPF"] != merged_df["PF"]) & ((merged_df["min_end"] - merged_df["max_start"] +1 )/(merged_df['qend'] - merged_df['qstart'] +1 ) > FalsePositiveThreshold)
